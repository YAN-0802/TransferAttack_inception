...
class Attack_dev(object):
    """
    Base class for all attacks.
    """
    def __init__(self, attack, model_name, model_dir, epsilon, targeted, random_start, norm, loss, device=None):
        """
        Initialize the hyperparameters
        ...
        if norm not in ['l2', 'linfty']:
            raise Exception("Unsupported norm {}".format(norm))
        self.attack = attack
        self.model = self.load_model(model_name, model_dir)
        self.epsilon = epsilon
        self.targeted = targeted
        self.random_start = random_start
        self.norm = norm
    ...
    def load_model(self, model_name, model_dir):
        ...
        print('=> Loading model {} from your path'.format(model_name))
    ...
    def get_loss(self, logits, label):
        ...
        """
        # Calculate the loss
        loss = self.loss(logits[0], label)  # logit
        loss += self.loss(logits[1], label)  # aux_logit
