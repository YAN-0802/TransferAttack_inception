...
def load_pretrained_model(cnn_model=[], vit_model=[]):
    ...
    for model_name in vit_model:
        filepath = timm.create_model(model_name).pretrained_cfg['hf_hub_id']
        pthname = os.path.basename(filepath)
        pathname = '/home/shuxue2/.cache/huggingface/hub/models--timm--' + pthname + '/pytorch_model.bin'
        yield model_name, timm.create_model(model_name, pretrained=True, pretrained_cfg_overlay=dict(file=pathname))
...
class AdvDataset(torch.utils.data.Dataset):
    def __init__(self, input_dir=None, output_dir=None, targeted=False, eval=False):
        ...
        if eval:
            ...
            self.data_dir = os.path.join(self.data_dir, 'val_rs')
...
class ImageNet(torch.utils.data.Dataset):
    ...
    def __init__(self, input_dir=None, csv_path=None, output_dir=None, targeted=False, eval=False, transforms=None):
        self.data_dir = input_dir
    ...
    def __getitem__(self, index):
        ...
        img_path = os.path.join(self.data_dir, ImageID)
